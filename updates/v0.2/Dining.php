 ...
 private function submit(array $d):array {
  ...
  return $this->txn(function()use($d,$normalized,$required,$hash){
   ...
   foreach($normalized as $key=>$qty){
    ...
    $s=$this->one('SELECT s.*,p.product_name,p.product_status,p.is_delete,p.app_id,p.shop_supplier_id FROM jjjfood_product_sku s JOIN jjjfood_product p ON p.product_id=s.product_id WHERE s.product_sku_id=? FOR UPDATE',[$id]);
